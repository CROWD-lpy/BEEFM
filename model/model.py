...
from utils.process_utils import get_bond_features
...
class RXN_Sequence(nn.Module):
    ...
    def _build_layers(self):
        ...
        self.w_b_out = nn.Sequential(
            nn.Linear(config['mpn_size'] * 2 - 2, config['mlp_size'] * 2),
            nn.ReLU(),
        ...
        self.w_g_out = nn.Sequential(
            nn.Linear(config['mpn_size'] - 1, config['mlp_size'] * 2),
            nn.ReLU(),
            nn.Linear(config['mlp_size'] * 2, config['mlp_size']),
            nn.ReLU(),
            nn.Dropout(p=config['dropout_mlp']),
    ...
    def calc_model(self, batch):
        ...
        atom_feats = self.w_a(batch.x[0])  # (n_atoms, config['mpn_size'] - 1)
        scores = []; prelabel = torch.zeros(atom_feats.size()[0], 1).to(dvice)

        for edit_idx in range(self.edit_length):
            bond_feats = self.w_b(batch.edge_attr[edit_idx])  # (n_bonds. config['mpn_size'])
            atom_feats = torch.cat([atom_feats, prelabel], dim=1)
            ...
            atom_feats = self.w_a3(atom_feats) # (n_atoms, config['mpn_size'] - 1)
    ...
    def predict(self, batch, mol, step_pred=8, edit_save=3):
        ...
        atom_feats = self.w_a(batch.x)
        bond_feats = batch.edge_attr
        bond_index = batch.edge_index

        results = []
        pre_model_test = [(mol, atom_feats, bond_index, bond_feats, torch.zeros(batch.x.size()[0], 1).to(self.device), [])]

        for edit_idx in range(step_pred):

            pre_model_test_update = []
            for pred_idx, pre_model_test_ in enumerate(pre_model_test):
                int_mol, atom_feats, bond_index, bond_feats, prelabel, pre_edits = pre_model_test_

                #-------------------------------------------------------------
                atom_feats = torch.cat([atom_feats, prelabel], dim=1)
                bond_feats = self.w_b(bond_feats)
                ...
                atom_feats = self.w_a2(
                    torch.cat([atom_feats_gnn, atom_feats_attn], dim=1))  # (n_atoms, config['mpn_size'])
                atom_feats, mask = creat_edits_feats(atom_feats, batch.ptr)
                ...
                for indice_idx, (value, indice) in enumerate(zip(values, indices)):
                    ...
                    if ans is not None:
                        ...
                        pre_model_test_update.append(
                            (mol_update, atom_feats.clone(), bond_index.clone(), bond_feats.clone(), prelabel_update, pre_edits + [edit])
                        )
                    else:
    ...
    def pred_step(self, batch):
        ...
        results = []
        pre_model_test = [(atom_feats, bond_index, bond_feats, torch.zeros(batch.x.size()[0], 1).to(self.device), [])]

        atom_feats, bond_index, bond_feats, prelabel, pre_edits = pre_model_test[0]

        # -------------------------------------------------------------
        atom_feats = torch.cat([atom_feats, prelabel], dim=1)
        bond_feats = self.w_b(bond_feats)
        edge_ptr = [0, bond_index.size()[1]]
        atom_feats_gnn = self.encoder_mpnn(atom_feats, bond_index, bond_feats,
                                           batch.ptr)  # (n_atoms, config['mpn_size'])
        atom_attn_pre, mask = creat_edits_feats(atom_feats, batch.ptr)
        _, atom_attn_aft = self.Attention(atom_attn_pre, mask)
        atom_feats_attn = unbatch_feats(atom_attn_aft, batch.ptr)
        ...
        bond_end = atom_feats[batch.edge_index_out[1, :]]
        bond_feats_outs = torch.cat([bond_start, bond_end], dim=1)
        bond_outs = self.w_b_out(bond_feats_outs)

        # print(bond_outs.size())
        return atom_feats, bond_outs